...
- [x] Boucle de denoising Euler flow-matching (`native/minimax_h3/sampling.py::run_minimax_h3_sampling` + `ASDX_MiniMaxH3Sampler`) : **volontairement autonome, pas integree a `sampler/core.py::_SamplerCore`** (2000+ lignes partagees par toutes les autres familles — teacache/kontext/LoRA schedule/identity edit/ControlNet/CFG, rien de tout ça ne s'applique a MiniMax H3, qui n'a pas de CFG (le workflow reel utilise `BasicGuider`, pas `CFGGuider`) et opere sur deux flux latents). Schedule de sigma verifie bit-exact contre le vrai `comfy.samplers.simple_scheduler`. **Graphe complet t2v desormais cablable de bout en bout** (ModelLoader+TextEncoderLoader -> TextEncode -> EmptyLatentAV -> Sampler -> VAEDecode/VAEDecodeAudio -> CreateVideo/SaveVideo) mais pas encore fume-teste comme graphe ComfyUI complet sur ce Mac (chaque brique individuelle est verifiee sur checkpoint reel ou reference reelle, l'assemblage complet ne l'est pas encore).
