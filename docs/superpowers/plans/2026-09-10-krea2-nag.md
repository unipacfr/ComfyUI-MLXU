...
def test_predict_nag_target_offset_matches_source_token_count():
    ...
    src_h, src_w = 2, 2
    img = mx.random.normal((1, img_h * img_w, dit.config.channels * dit.patch ** 2))
    src = mx.random.normal((1, src_h * src_w, dit.config.channels * dit.patch ** 2))
    context = dit.encode_text(mx.random.normal((1, 4, dit.txtlayers * dit.txtdim)))
    t = mx.array([0.5])
    ...
    assert out_a.shape == (1, img_h * img_w, dit.config.channels * dit.patch ** 2)
    assert not np.allclose(np.array(out_a), np.array(out_b))
```

Check `Krea2Config`'s and `get_rope_grid`'s real signatures (`config.py`, `model.py:680-742`) before finalizing this test — the exact keyword names must match; do not guess.
