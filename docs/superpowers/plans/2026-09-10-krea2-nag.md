...
- Identity Edit: `ref_boost` applies only to the positive pass; NAG combine applies only to target-image tokens (`guide_attention_tail`), never to source-reference tokens.
...
- Produces: `_nag_edit_block(block, positive_text, negative_text, image, target_offset, vec, positive_freqs, negative_freqs, ref_boost, phi, tau, alpha) -> tuple[mx.array, mx.array, mx.array]`. `image` here is `[source | target]` already concatenated by the caller (Task 6); `target_offset` = source token count. Consumed by Task 6.
