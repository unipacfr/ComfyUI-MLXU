...
## Every ported family's bridge must apply its LatentFormat's per-channel denormalization
kind: convention | date: 2026-09-23 | status: canon
Any model whose `comfy.latent_formats.LatentFormat` subclass defines `latents_mean`/`latents_std` (or overrides `process_out`, even when `scale_factor` looks like identity) samples in a normalized ("whitened") latent space -- its `mlx_to_comfy_latent_<family>` bridge function must apply that family's `* std + mean` de-whitening before handing samples to VAE decode, matching `comfy/samplers.py:1238`'s unconditional `process_latent_out` call. Hit twice: Krea2/Wan21 (`native/config.py::process_wan21_latent_out`, "previously misread as an identity no-op") and Qwen Image 2.1 (missing entirely, produced a systematic grid/mesh artifact on every generation -- out-of-distribution latents feeding the VAE decoder's upsampling layers -- fixed commit ea3af2f). Because: this step lives entirely in ComfyUI's sampler wrapper, never mentioned in the DiT's own `model.py`, so it's invisible when porting a family by reading only the transformer reference; check `comfy/latent_formats.py::<Family>` for it explicitly before considering any new family's sampling path done, and test it by value (zero DiT output must denormalize to exactly `latents_mean`), not just by output shape.
